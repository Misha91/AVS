...
void clearLCDS(vu32 speed, char led){
    uint8_t p, c;
  //LCD_SendByte(CMD_SET_DISP_START_LINE, 1);
  for(p = 0; p < 8; p++) {
...
void clearLCDV(vu32 speed, char led){
    uint8_t p, c;
  //LCD_SendByte(CMD_SET_DISP_START_LINE, 1);
    ...
    for(c = 0; c < 132; c++) {         
        for(p = 0; p < 8; p++) {
            LCD_SendByte(CMD_SET_COLUMN_LOWER | (c & 0xf), 1);
            LCD_SendByte(CMD_SET_COLUMN_UPPER | ((c >> 4) & 0xf) , 1); 
            LCD_SendByte(CMD_SET_PAGE | p, 1);    
            LCD_SendByte(0x00, 0);
        }
    if (led) STM_EVAL_LEDOn(LED5);
...
void vertLine(vu32 speed, char led){
    uint8_t p, c;
  //LCD_SendByte(CMD_SET_DISP_START_LINE, 1);
...
void sawLine(vu32 speed, char led){
    uint8_t p, c;
    //LCD_SendByte(CMD_SET_DISP_START_LINE, 1);
    uint8_t k = 0;
        ...
        for (c = 3; c < 132; c++){
            for(p = 0; p < 8; p+=2) {
                ...
                LCD_SendByte(CMD_SET_COLUMN_UPPER | ((c >> 4) & 0xf) , 1);
                LCD_SendByte(CMD_SET_PAGE | p, 1);
                if (k) LCD_SendByte(1 << k, 0);              
                else LCD_SendByte(0xFF, 0);
...
void drawRect(vu32 speed){
    LCD_SendByte(CMD_SET_PAGE | 0, 1);
    LCD_SendByte(CMD_SET_COLUMN_LOWER | (19 & 0xf), 1);
    LCD_SendByte(CMD_SET_COLUMN_UPPER | ((19 >> 4) & 0xf) , 1);
    uint8_t c = 0;
    for (c = 0; c < 98; c++){
        LCD_SendByte(0x80, 0);
    }
    delay(speed);
    for (c = 1; c < 7; c++){
        LCD_SendByte(CMD_SET_PAGE | c, 1);
        LCD_SendByte(CMD_SET_COLUMN_LOWER | (19 & 0xf), 1);
        LCD_SendByte(CMD_SET_COLUMN_UPPER | ((19 >> 4) & 0xf) , 1);
        LCD_SendByte(0xFF, 0);
        LCD_SendByte(CMD_SET_COLUMN_LOWER | (20 + 16*(c-1) & 0xf), 1);
        LCD_SendByte(CMD_SET_COLUMN_UPPER | ((20 + 16*(c-1) >> 4) & 0xf) , 1);
        ...
        LCD_SendByte(CMD_SET_COLUMN_LOWER | (116 - 16*(c) & 0xf), 1);
        LCD_SendByte(CMD_SET_COLUMN_UPPER | ((116 - 16*(c) >> 4) & 0xf) , 1);
        LCD_SendByte(0x80, 0);
        LCD_SendByte(0x80, 0);
        LCD_SendByte(0x40, 0);
        LCD_SendByte(0x40, 0);
        ...
        LCD_SendByte(CMD_SET_COLUMN_LOWER | (116 & 0xf), 1);
        LCD_SendByte(CMD_SET_COLUMN_UPPER | ((116 >> 4) & 0xf) , 1);
        LCD_SendByte(0xFF, 0);
        delay(speed);
    }
    
    LCD_SendByte(CMD_SET_PAGE | 7, 1);
    LCD_SendByte(CMD_SET_COLUMN_LOWER | (19 & 0xf), 1);
    LCD_SendByte(CMD_SET_COLUMN_UPPER | ((19 >> 4) & 0xf) , 1);   
    for (c = 0; c < 98; c++){
        LCD_SendByte(0x1, 0);
